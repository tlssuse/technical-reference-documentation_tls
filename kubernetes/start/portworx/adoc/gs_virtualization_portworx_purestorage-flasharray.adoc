...
.. Enter the desired *Size_ of the disk to be mapped to the SUSE Virtualization cluster.
...
The {pxo} will efficiently manage the installation and upgrade workflow of all {pxe} components.
To deploy the {pxo}, you will use the first of the Kubectl commands you generated in <<Generate the Portworx spec>>.
Now, you use the Kubectl commands of the generated Portworx spec you created in the previous section to deploy the {pxo}.
...
[NOTE]
====
Editor: Consider simplifying this content by referencing the existing product documentation section and ONLY detailing the differences required to use Portworx.
====
...
. Select the namespace for your VM.
+
[NOTE]
====
...
. (Optional) Select a VM template (iso-image, raw-image, or windows-iso-image) to speed up provisioning.

. On the *Basics* tab, configure your VM with the following settings:
+
--
* CPU: 2
+
...
* (Optional) Overcommit configuration: You may set CPU, memory, and storage overcommit ratios to allow scheduling of additional VMs even when physical resources are fully utilized.

* (Optional) Other advanced settings: These include run strategy, OS type, cloud-init data, etc.
--

. On the *Volumes* tab, select or add volumes as follows:


=== Verify VM Storage

You can use the {svirt} UI or CLI to verify that the VM is backed by Pure Storage with the Portworx CSI.
...
. Log in to the SUSE Virtualization management CLI as root.
...
This verifies that your VM is backed by the {pfa} with {pxe}.
...
* {svirt-docs}[SUSE Virtualization documentation]
