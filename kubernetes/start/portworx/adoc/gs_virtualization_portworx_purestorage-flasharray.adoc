...
{svirt-website}[{svirt}]::
{svirt} (formerly Harvester) is a modern, open, interoperable, hyperconverged infrastructure (HCI) solution built on Kubernetes.  {svirt} leverages other components (such as {slmicro-website}[{slmicro}] and {rke2-docs}[{rke2}]) to deliver a secure, resilient, and scalable platform for managing virtual machine and container workloads.
Access the {svirt-docs}[{svirt} documentation] for detailed technical information, including {svirt-reqs}[Hardware and Network Requirements] and installation guidance.  This guide references {svirt} v1.5.1 and later.
...
This guide references {pxe-docs}[{pxe}] 3.4.0 or later and Portworx Operator 25.2.2 or later.


{pfa-long}::
{pfa} delivers high-performance, scalable, all-flash storage.
